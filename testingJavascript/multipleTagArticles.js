...
const multipleTagArticles = articlesArray
  .map(array => {
    if (array.length === 5) {
      const tags = array[4].split(",");
      if (tags.length === 1) {
        return null;
      } else {
        return ` The article title : "${array[2]}"`;// and it has multiple tags, id "${array[4]}"
...
multipleTagArticles.forEach((article, i) => {
  if (article) {
